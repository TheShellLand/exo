...
apt install -y software-properties-common 1>>$log 2>>$err && let progress++
add-apt-repository -y ppa:noobslab/macbuntu 1>>$log 2>>$err 
apt update 1>>$log 2>>$err 

apt install -y gnome-tweak-tool 1>>$log 2>>$err && let progress++
apt install -y ubuntu-gnome-desktop 1>>$log 2>>$err && let progress++
#apt install -y ubuntu-desktop 1>>$log 2>>$err && let progress++

apt install -y plank 1>>$log 2>>$err && let progress++
apt install -y macbuntu-os-plank-theme-lts-v8 1>>$log 2>>$err && let progress++
apt install -y macbuntu-os-icons-lts-v8 1>>$log 2>>$err && let progress++
apt install -y macbuntu-os-ithemes-lts-v8 1>>$log 2>>$err && let progress++
apt install -y slingscold 1>>$log 2>>$err && let progress++
#apt install -y albert 1>>$log 2>>$err && let progress++

apt install -y libreoffice-style-sifr 1>>$log 2>>$err && let progress++
fonts=mac-fonts.zip
if [ ! -f $apps/$fonts ]; then
	wget -q -O $apps/$fonts http://drive.noobslab.com/data/Mac/macfonts.zip
	unzip $apps/$fonts -d /usr/share/fonts 1>>$log 2>>$err && let progress++
	#rm $apps/$fonts
else
	unzip -o $apps/mac-fonts.zip -d /usr/share/fonts 1>>$log 2>>$err && let progress++
fi
fc-cache -f -v  1>>$log 2>>$err && let progress++
