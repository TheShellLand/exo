...
*** SMARTOS : Change hostname
    ...
    $ sm-set-hostname the-new-hostname
...
*** SMARTOS : Remove offending log files
    logadm -r /zones/<uuid>/root/tmp/vm.log

*** SMARTOS : See all logs in logadm
    logadm -V
...
*** SMARTOS : vmadm destroy doesn't tell logadm to stop rotating logs
    for v in $(logadm -V |  awk -F/ '/zones/ {print $3}'); do exist=$(vmadm lookup uuid=$v); [[ -z "$exist" ]] && logadm -r /zones/$v/root/tmp/vm.log; done

*** SMARTOS : mdb 0 analyzing system dump
